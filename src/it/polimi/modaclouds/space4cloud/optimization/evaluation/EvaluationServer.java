...
public class EvaluationServer implements ActionListener{
	...
	public EvaluationServer(String solver) {
		//set min to 24
		executor = new ThreadPoolExecutor(24, nMaxThreads, 200, TimeUnit.MILLISECONDS, queue);
	...
	public void evaluateInstance(Instance instance, String solver){
		...
		if(!instance.isEvaluated()){ 
			SolutionEvaluator eval = new SolutionEvaluator(instance,solver,null);
	...
	public void EvaluateSolution(Solution sol){
		...
		if(!sol.isEvaluated()){
			...
			for(Instance i:instanceList){
				...
				if(!i.isEvaluated()){  
					SolutionEvaluator eval = new SolutionEvaluator(i,solver,sol);				
					eval.addListener(this);
					if(solver.equals(MessageStrings.LQNS_SOLVER))
					...
					counters.put(sol,counters.get(sol)+1);//if the application has already been evaluated increment the counter 
			}
	...
	public synchronized void actionPerformed(ActionEvent e) {
		...
		if(e.getActionCommand() != null && e.getActionCommand().equals("ResultsUpdated")){
	...
	private void incrementTotalNumberOfEvaluations(){
		...
		pcs.firePropertyChange("totalNumberOfEvaluations", temp, totalNumberOfEvaluations);
	}
