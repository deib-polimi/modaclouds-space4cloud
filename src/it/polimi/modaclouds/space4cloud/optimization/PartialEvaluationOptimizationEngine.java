...
public class PartialEvaluationOptimizationEngine extends OptEngine{
	...
	/**
	 * For each hourly application perform a scale in of a random resource with a descending factor that starts from the default value and ends to 1. 
	 * @param sol
	 * @return true is the solution has been improved, false if no resources could be scaled
	 */
	private boolean noImprovementPhase(Solution sol){
		...
		for(int iterations=0;iterations<MAX_NUMBER_OF_ITERATIONS;iterations++){
			...
			for(int hour=0;hour<24;hour++){
				...
				//if no resource can be scaled in in this hour then jump to the next one
				if(vettResTot.get(hour).size() ==  0)
					continue;

				
				//if there are resources that can be scale then chose one randomly
				res = vettResTot.get(hour).get(random.nextInt(vettResTot.get(hour).size()));
			...
			//if there was no scale in for any of the 24 hours increase the number of iterations with no improvement and exit
			if(noScaleIn){
				numIterNoImprov++;
				return false;
			}
			...
			//if an application has become feasible, revert it and try again with a smaller factor
			for(int i=0;i<24;i++){				
				if(!sol.getApplication(i).isFeasible()){
					sol.copyApplication(previousSol.getApplication(i),i);
					factors[i] = DEFAULT_SCALE_IN_FACTOR  - ((iterations+1)/MAX_NUMBER_OF_ITERATIONS)*(DEFAULT_SCALE_IN_FACTOR - 1);
				}
			}
